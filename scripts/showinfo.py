...
p = image.read(0,sysinfo+6)
a = image.read(0,sysinfo+4)+image.read(0,sysinfo+5)*256
print("Image boots to            ${0:02x} ${1:04x}".format(p,a))
a = image.read(0,sysinfo+16)+image.read(0,sysinfo+17)*256
print("Z80 stack is at           ${0:04x}".format(a))

pageUsageTable = image.read(0,sysinfo+8)+image.read(0,sysinfo+9	)*256
pageCurrent = 0x20
pageUsage = { 0:[], 1:[] , 2:[], 3:[] }
